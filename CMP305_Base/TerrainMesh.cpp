...
TerrainMesh::TerrainMesh( ID3D11Device* device, ID3D11DeviceContext* deviceContext, int lresolution, float yoff ) :
	PlaneMesh( device, deviceContext, lresolution ) 
{
	Resize( resolution,yoff );
	...
	for( int j = 0; j < ( resolution ); j++ ) {
		for( int i = 0; i < ( resolution ); i++ ) {
			...
			for(int k =0; k<10;++k)
			{
				...
				heightMap[j][i] += (1-std::abs(PerlinNoise::PerlinNoise::noise((i  * std::pow(2, k) * scale) / (50.f), (j  * std::pow(2, k) * scale) / (50.f)))) *(20/std::pow(2,k));

				//heightMap[j][i] += PerlinNoise::PerlinNoise::noise((i * std::pow(2, k) * scale) / (50.f), (j * std::pow(2, k) * scale) / (50.f)) * (20.f / std::pow(2, k));
			}
			//heightMap[j][i] += 1;
			heightMap[j][i] += yoffset;
...
void TerrainMesh::Resize( int newResolution, float yoff ) {
	yoffset = yoff;
...
void TerrainMesh::Regenerate( ID3D11Device * device, ID3D11DeviceContext * deviceContext, bool smooth ) {
